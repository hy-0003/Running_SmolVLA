...
"""
import sys
sys.path.insert(0, r"C:\Users\86182\Desktop\Skill-Stack-Unlock-Plan\LIBERO")
...
def eval_libero(args: Args) -> None:
    ...
    config = decode(PreTrainedConfig, config_dict)

# --- 创建策略 ---
    policy = SmolVLAPolicy(config)

# --- 加载模型权重 ---
    weights_path = os.path.join(args.policy_path, "model.safetensors")
    if os.path.exists(weights_path):
        state_dict = safetensors_load(weights_path)
    else:
        state_dict = torch.load(os.path.join(args.policy_path, "pytorch_model.bin"), map_location="cpu")
    policy.load_state_dict(state_dict, strict=False)

# --- 加载归一化统计量 ---
    stats_path = os.path.join(args.policy_path, "policy_preprocessor_step_5_normalizer_processor.safetensors")
    if os.path.exists(stats_path):
        stats_state_dict = {}
        with safe_open(stats_path, framework="pt", device="cpu") as f:
            for key in f.keys():
                stats_state_dict[key] = f.get_tensor(key)
    # 直接加载到 normalizer 模块
        policy.normalize_inputs.load_state_dict(stats_state_dict, strict=False)
        print("Normalization stats loaded from safetensors.")
    else:
    # 尝试其他可能的统计量文件
        alt_stats = os.path.join(args.policy_path, "stats.json")
        if os.path.exists(alt_stats):
            with open(alt_stats, "r") as f:
                stats = json.load(f)
        # 根据 Normalize 模块的接口设置（可能需要调用特定方法）
            if hasattr(policy.normalize_inputs, "set_normalizers"):
                policy.normalize_inputs.set_normalizers(stats)
            else:
                # 直接赋值统计量张量（常见实现）
                for key, val in stats.items():
                    if isinstance(val, dict):
                        mean = torch.tensor(val["mean"])
                        std = torch.tensor(val["std"])
                    # 根据 Normalize 模块内部结构设置
                    # 通常有名为 "mean" 和 "std" 的缓冲区
                        if hasattr(policy.normalize_inputs, key):
                            getattr(policy.normalize_inputs, key).mean.copy_(mean)
                            getattr(policy.normalize_inputs, key).std.copy_(std)
            print("Normalization stats loaded from stats.json (fallback).")
        else:
            print("Warning: No normalization stats found, model may perform poorly.")
